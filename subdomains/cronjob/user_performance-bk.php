...
require_once  CMS_INC_ROOT . DS . 'UserMonthRanking.class.php';
...
if (!empty($users)) {
    ...
    //$rankings = ArticleRanking::getAllByParam(array('user_id' => $user_id_arr));

    $max_monthes = UserMonthRanking::getMaxMonthUserList($user_id_arr);
    ...
    while ($start_time < $end_time) {
        ...
        $start_date = date("Y-m-01 00:00:00", $start_time);
        $result = ArticleRanking::generateMonthReport($conditions);
        foreach ($result as $user_id => $user) {
            $user['report_month'] = $month;
            UserMonthRanking::store($user);
        }
    }
}
?>