...
class GoogleSpell extends SpellChecker {
	...
	function &checkWords($lang, $words) {
        ...
        if (!empty($this->_config['general.personal_word_list'])) {
            $wordlist = file_get_contents($this->_config['general.personal_word_list']);

            for ($i=0; $i<count($matches); $i++) {
                $word = $this->_unhtmlentities(mb_substr($wordstr, $matches[$i][1], $matches[$i][2], "UTF-8"));
                $pos = strripos($wordlist, $word."\n");
                if ($pos === false) {
                    $words[] = $word;
                } else {
                    continue;
	 ...
	 *************************************************************/
	function addWordToPersonalDict($lang, $word)
	{
        if (empty($this->_config['general.personal_word_list'])) {
            return false;
        }
        ...
        //$lang = empty($lang) ? "en" : $lang; // do nothing;
        $wordlist = file_get_contents($this->_config['general.personal_word_list']);
        $pos = strripos($wordlist, $word."\n");
        if ($pos === false) {
            if (file_put_contents($this->_config['general.personal_word_list'], $word."\n", FILE_APPEND)) {
                return true;
            } else {
                return false;
            }
