...
function getEditorCount(ed)
{
    ...
    htmlNode.innerHTML = ed.getContent();
    if (htmlNode.innerText) {
        html = htmlNode.innerText; // IE
    }
    html = htmlNode.textContent; // FF
    var text = html.replace(/<\/?[^>]+>/gi,"").replace(/&nbsp;|&#160;|\.|:/gi, ' ').replace(/\s+/g," ");
    var arr = text.split(' ');
    var len = (text.length === 0) ? 0 : (arr.length);
    if (len > 0 && arr[len-1] == '') {
        len--;
    }
    jQuery('#' + tinyMCE.activeEditor.id + '_wordcount').html("<span>" + len + "</span> words, <span>" + text.length + "</span> characters");
    return len;
}
...
function check_f_basic(f)
{
  ...
  if (f.pay_pref.value.length == 0 && f.pay_pref.value == 3 && f.paypal_email.value.length == 0) {
      alert("Please specify the paypal email");
...
/*jQuery('input[name="samples[fileField][]"]').each(function(index) {
  jQuery()
...
function submitTabOne()
{
  var f = document.f_candidate;
  var msg = 'Please fill out your Basic Information';
  if (f.first_name.value.length == 0) {
      alert(msg);
      f.first_name.focus();
      return false;
...
//  }
  ...
  /*if (f.clocation.value == '') {
      alert('Please mention if you are a citizen of one of these countries');
      f.clocation.focus();
      return false;
  }*/
/*  if (!f.work_in_us.checked) {
      //alert('Please mention if you are authorized to work in the US');
      alert(msg);
      f.work_in_us.focus();
      return false;
  }*/
...
function submitTabTwo()
{
  var f = document.f_candidate;
  var msg = 'Please fill all the fields to continue';
  var pids = document.getElementsByName('categories[parent_id][]');
  var cateids = document.getElementsByName('categories[category_id][]');
  var levels = document.getElementsByName('categories[level][]');
  var catefiles = document.getElementsByName('categories[fileField][]');
  ...
  var slinks = document.getElementsByName('samples[link][]');
  var files = document.getElementsByName('samples[fileField][]');
  var portfolios = document.getElementsByName('samples[portfolio][]');
  var plinktypes = document.getElementsByName('plinks[type][]');
  var plinks = document.getElementsByName('plinks[value][]');
  var writerlevels = document.getElementsByName('writer_level[]');
  f.opt_index.value = arguments[1]||4;
  ...
  if (!has_one) {
      ...
      if (!has_one) {
          ...
          has_one = true;
          for (var i=0;i<len; i++) {
              if (slinks[i].value != 'http://www.' || files[i].value != '') {
                  has_one = true;break;
              }
          }
          if (!has_one) {
              alert('Please provide a sample of your best work.');
              jQuery(slinks[0]).focus();
              return false;
          } else {
              has_one = false;
              ...
              for (var i=0;i<len; i++) {                 
                ...
                if (pids[i].value == 'Select' || pids[i].value.length==0) {
                    catefiles[i].value = '';
                    continue;
                } else if (pids[i].value.length >  0 && ((levels[i].value.length  == 0 && catelinks[i].value.length <= 12 &&  catefiles[i].value.length == 0) || (cateids[i].value == 0 || cateids[i].value.length == 0) && olength > 1)) {
                  if (olength > 1 && (cateids[i].value.length  == 0 || cateids[i].value == 0)) {
                      ...
                      alert('Please specify an URL, Document or Relevant Experience');
                      jQuery(catelinks[i]).focus();
                  }
                  return false;
                } else if (pids[i].value.length >  0 && (cateids[i].value  > 0 && olength > 1 || olength == 1) && (levels[i].value.length  > 0 || catelinks[i].value.length >  12 || catefiles[i].value.length > 0)){
                    has_one = true;
              ...
              if (!has_one) {
                  alert('You must choose at least one speciality. ');
                  jQuery(pids[0]).focus()
                  return false;
              }
          }
...
function changeValue(checkbox){
	if(document.getElementById("AcceptButton").value == "Deny"){
		document.getElementById("AcceptButton").value = "Accepted";
	}
	else{
		document.getElementById("AcceptButton").value = "Deny";
	}
}

function submitTabTwoNew()
{
	/*
	var f = document.f_candidate;
	f.opt_index.value = arguments[1]||4;
	*/
	var checkedValue = document.getElementById("AcceptButton").value;
	if(checkedValue == "Accepted"){
		showTab(2);
	}
	else{
		alert('Please agree to the contractor agreement.');
	}
}
...
function ajax_uploaded(formid, url) 
{
    ...
    jForm.submit(function( objEvent ){
        ...
        jFrame.load(function( objEvent ){
            ...
            //alert(objData); return false;
            if (isDigit(objData)) {
                str = '<script type="text/javascript">jQuery(\'input[name="candidate_id"]\')).val(' + objData  + ');alert("Invalid file, please to check");</script>';
            } else {
                // str = '<script type="text/javascript">jQuery("#div-allforms").html("<div style=\"color:red\">' +objData+ '</div>")</script>';
                 //str = '<script type="text/javascript">jQuery("#div-allforms").css("color", "red").html("'+ objData + '");</script>';
                 str = "<script type=\"text/javascript\">jQuery(\"#div-allforms\").css(\"color\", \"red\").html('"+ objData + "');</script>";
            }
            jQuery('#ajaxresult', window.parent.document).html( str);
            //jQuery('#ajaxresult', window.parent.document).html('here');
            //alert(jQuery('#ajaxresult', window.parent.document).html());
...
//added by nancy xu 2012-10-08 16:11
function experience_type_Change(val, index)
{
//    var val = jQuery("#categories_experience_type_"+index).val();
//    alert(val);
    switch(val) {
    case '1':
        jQuery("#categories_link_"+index).show();
        jQuery("#categories_filename_"+index).hide();
        jQuery("#categories_level_"+index).hide();
        break;
    case '2':
        jQuery("#categories_link_"+index).hide();
        jQuery("#categories_filename_"+index).show();
        jQuery("#categories_level_"+index).hide();
        break;
    case '3':
        jQuery("#categories_link_"+index).hide();
        jQuery("#categories_filename_"+index).hide();
        jQuery("#categories_level_"+index).show().css('vertical-align', 'middle');
        break;
    }
}
//end