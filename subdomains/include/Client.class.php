...
class Client {
       ...
       if (User::getRole() != 'agency' && isset($p['agency']) && $p['agency'] > 0) {
        ...
        if ($rs) {
            ...
            while (!$rs->EOF) {
                ...
                $fields['total_delivered'] = '0';
                $fields['pct_total_delivered'] = '0%';
                $result[$i] = $fields;
                $campaign_ids[$i] = $fields['campaign_id'];
                $rs->MoveNext();
                $i++;
    ...
    function getReportInfo(&$result, $month, $ids, $conditions, $q, $group_field, $is_home = false)
    {
        ...
        Client::getCountGroupByClients($result, 'total_delivered', $ids, array_merge($conditions,array("ar.delivered_date IS NOT NULL")), $q, $group_field);
    }
