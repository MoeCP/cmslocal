...
class DomainTag {
    ...
    {
        $item = $rs = array();
        if (isset($result['tagName'])) $result = array($result);
        foreach ($result as $row) {
            $tag_name = trim($row['tagName']);
            $tag_id = trim($row['tagId']); 
            if (strlen($source) || isset($row['source']) && strlen($row['source'])) {
                $source = $row['source'];
                ...
                $error = 'Please specify the domain ID when you use the Master API';
            }
            if (empty($error)) {
