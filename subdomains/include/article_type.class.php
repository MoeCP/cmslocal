...
class ArticleType{
	...
	public static function store( $hash )
	{
        ...
        if (isset($hash['is_inactive']))  {
            if ($hash['is_inactive'] == 1) {
                $sql = "SELECT COUNT(keyword_id) FROM  campaign_keyword WHERE article_type = '" . $type_id . "'" ;
                $count = $conn->GetOne($sql);
                if ($count > 0) {
                    $feedback  = "This article type was used, you can't hide this article type.";
                    return false;
                }
            }
            $bind['is_inactive'] = mysql_escape_string(htmlspecialchars(trim($hash['is_inactive'])));
        }
		...
		foreach ($bind as $k => $value)
		{
			...
			case 'type_name':
                ...
                {
                    if(!self::checkTypeNameUnique($value, $hash['type_id']))
                    {
    ...
    {   
        ...
        if (count($condition))
            ...
            $sql .= " WHERE 1=1 ";
        if (isset($p['is_inactive']) && $p['is_inactive'] >= 0) {
            $sql .= " AND is_inactive = {$p['is_inactive']}";
        } else {
            $sql .= " AND is_inactive <> 1 ";
        }
        if (!empty($orderby)) {
