...
class Article {
    ...
    function getInfo($article_id, $is_contain_comment = false, $is_tag=true)
    {
        ...
        $q = "SELECT ar.*, aaf.small_image,aaf.large_image, aaf.image_credit, aaf.image_caption, aaf.meta_description, aaf.blurb, aaf.category_id, cc.campaign_name, cc.campaign_id, cc.template, cc.show_cp_bio, cl.client_id, cl.project_manager_id, ck.* ".
             "FROM articles AS ar ".
    ...
    function setNoflowStatus($article_id, $status)
    {
        global $conn, $mailer_param, $g_tag, $feedback, $admin_host;

        $sql = "UPDATE articles SET noflow_status=". $status . " WHERE article_id=" . $article_id;
        
        $conn->Execute($sql);
        if ($conn->Affected_Rows()) 
		{
            $articleinfo = self::getInfo($article_id);
            $host = client_is_loggedin() ? "https://" . $admin_host : "https://clients.copypress.com";
            $url = $host . "/article/approve_article.php?article_id={$articleinfo['article_id']}&keyword_id={$articleinfo['keyword_id']}&campaign_id={$articleinfo['campaign_id']}";
            $subject = "Article Status changed";
            $body = "The article ".$articleinfo['title']." status has changed to '".$g_tag[$status]."'. To view these changes, please visit <a href=\"{$url}\" >this article.</a>";

            if (client_is_loggedin()) {
                if ($articleinfo['project_manager_id'] > 0 && in_array((int)$status, array(10,30))) {
                    $uinfo = User::getInfo($articleinfo['project_manager_id']);
                    $address = $uinfo["email"];
                    send_smtp_mail($address, $subject, $body, $mailer_param);
                }
            } else {
                if ($articleinfo['client_id'] > 0 && in_array((int)$status, array(20))) {
                    $uinfo = Client::getInfo($articleinfo['client_id']);
                    $address = $uinfo["email"];
                    send_smtp_mail($address, $subject, $body, $mailer_param);
                }
            }
    ...
    function setInfo($p = array())
    {
        ...
        */
        ...
        // add by nancy xu 2013-09-23 19:59
        // add/edit article additional fields
        require_once CMS_INC_ROOT . '/article_additional_field.class.php';
        $oField = new ArticleAdditionalField;
        $oField->storeFromArticle($p);
        // end
...
//        $cost_for_article = $cost_per_article * $word_count;
    ...
    function downloadArticleByCampaignID($campaign_id, $p = array(), $is_tag = true)
    {
        ...
        $q = "SELECT ar.*, ck.* , aaf.small_image,aaf.large_image, aaf.image_credit, aaf.image_caption, aaf.meta_description, aaf.blurb,aaf.category_id, cc.campaign_name, cc.show_cp_bio, u.user_name AS creator , uc.user_name AS author ".
             "\nFROM articles AS ar ".
             "\nLEFT JOIN campaign_keyword AS ck ON (ck.keyword_id = ar.keyword_id) ".
            "LEFT JOIN article_additional_fields AS aaf ON (ar.article_id = aaf.article_id) ".
             "\nLEFT JOIN users AS u ON (u.user_id = ar.creation_user_id) ".
...
//            }
    ...
    function generateArticleHistorySql($version_history_id, $old_article)
    {
        ...
        unset($hash['tag_name']);
        unset($hash['template']);
        unset($hash['image_credit']);
        unset($hash['image_caption']);
        unset($hash['small_image']);
        unset($hash['large_image']);
        unset($hash['category_id']);
        unset($hash['meta_description']);
        unset($hash['blurb']);
        unset($hash['qaer_id']);
        /**
         * @since 2015-12-10 jkuster remove noflow_status from the hash as the articles_version_history table doesn't have it.
         */
        unset($hash['noflow_status']);
    ...
    */
    ...
    function getCheckedArticle($p = array(), $is_tag = true) 
    {
        ...
        // $q = "SELECT ar.*, ck.keyword, ck.copy_writer_id, ck.keyword_meta, ck.description_meta, ck.mapping_id ".
         $q = "SELECT ar.*, ck.*, aaf.small_image,aaf.large_image, aaf.image_credit, aaf.image_caption, aaf.meta_description, aaf.blurb, aaf.category_id, cc.show_cp_bio,cc.template ".
             "FROM articles AS ar ".
             "LEFT JOIN article_additional_fields AS aaf ON (ar.article_id = aaf.article_id) ".
             "LEFT JOIN campaign_keyword AS ck ON (ck.keyword_id = ar.keyword_id) ".
             "LEFT JOIN client_campaigns AS cc ON (cc.campaign_id = ck.campaign_id) ".
    ...
    function sendAnnouceMail($action = "reject", $keyword_id, $comment = '' )
    {
        ...
        */
        ...
        if ($action == "reject" || $action == 'approval') {
            ...
            if (user_is_loggedin()) {//editor reject.
                ...
                $q = "SELECT u.email, u.first_name, ck.keyword, ck.keyword_id, ar.article_id, cc.campaign_name, cc.campaign_id, ar.article_number, cp.user_name as cp_name, cp.role AS cp_role, u.role AS editor_role, ck.editor_id, ck.copy_writer_id, ct.user_name as ct_name ".
                     ...
                     "WHERE ck.keyword_id = '".$keyword_id."'" . $u_qw;
                if ($action == "reject") {
                    //##$mailer_param['cc'][0] = "cptech@copypress.com";
                    $event_id = 5;
                    $cc_event_id = 6;
                }
            }
        ...
        if (!empty($address)) {
            ...
            //} else if ($action == 'approval' && $comment != '') {//changed by leo @2016-05-25
                ...
                if (user_is_loggedin()) {
                    if (empty($comment)) {
                        ...
                        $body .= "Your article has been approved. <br />";
                    } else {
                        $subject = "Your article has been approved with editor comments";
...
//            }
    ...
    */   
    ...
    function sentComments($p, $comment)
    {
       ...
       $mailer_param['reply_to']  = (client_is_loggedin()) ? (Client::getEmail()):(User::getEmail());
       $mailer_param['from']  = $mailer_param['reply_to'];
        if (empty($comment)) {
            $feedback = 'Please input the comment';
            return false;
        }
        $comment = stripslashes(htmlspecialchars_decode(trim($comment)));
    ...
    CopyPress ";
            ...
            foreach ($users as $k => $user) {
                if (user_is_loggedin() && $user['user_id'] == User::getID()) {continue;}
                if (!empty($user)) {
...
//        $sql = "SELECT * FROM `articles_version_history` WHERE article_id={$article_id} AND  article_status =4 AND version_number = {$action['version']}";
    ...
    function deliverArticle($p)
    {
        global $conn;
        $article_id = $p['article_id'];
        $qws = '';
        if (is_array($article_id)) {
            $qws = 'where article_id in (' . implode(",", $article_id). ')';
        } else {
            $qws = 'WHERE article_id = ' . $article_id;
        }
        $sql = 'UPDATE articles SET delivered_date=\'' . date("Y-m-d H:i:s") . '\' '. $qws ;
        $conn->Execute($sql);
    }
}//end class Article
?>