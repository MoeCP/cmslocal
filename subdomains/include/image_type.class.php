...
class ImageType{
	...
	public static function store( $hash )
	{
		...
		if (count($bind))
		{
            ...
            if (strlen($bind['type_id']) == 0) {
                $bind['type_id'] = $conn->GenID('seq_image_type_type_id');
            }  else {
			...
			$sql      = "REPLACE INTO  `image_type` ({$fields}) VALUES ({$values}) ";
            $conn->Execute($sql);
            
            if (!isset($old_parent_id)&&$parent_id>=0 || $old_parent_id != $parent_id) {
                if (isset($old_parent_id) && $old_parent_id >= 0) {
                    $sql = "UPDATE `image_type` set total_nodes= total_nodes -1 WHERE  total_nodes > 0 and type_id=" . $old_parent_id;
                    $conn->Execute($sql);
                }
                if ($parent_id >= 0) {
                    $sql = "UPDATE `image_type` set total_nodes= total_nodes +1 WHERE   type_id=" . $parent_id;
                    $conn->Execute($sql);
                }
            }
    ...
    {
        global $conn;
        $sql = 'SELECT count( * ) AS total, parent_id FROM `image_type` WHERE 1 GROUP BY parent_id';
        $result = $conn->GetAll($sql);
        $pids = array();
        foreach ($result as $k => $row) {
            $pids[] = $row['parent_id'];
            $sql = "UPDATE `image_type` set total total_nodes";
        }
    }
    ...
    {   
        ...
        $sql = "SELECT {$columns} FROM `image_type` ";
        if (count($condition))
