...
class ArticleAction{
    ...
    //added By Leo 6/6/2015
    function getSumByArticleStatus($user_id=0, $campaign_id=0, $article_status='1gc', $role='editor') {
        global $conn;

        $role_field = ($role && ($role == 'copy writer') ) ? 'ck.copy_writer_id' : 'ck.editor_id';

        $q = "SELECT COUNT(ck.keyword_id) AS artotal FROM campaign_keyword AS ck ";
        $q .= "LEFT JOIN articles AS ar ON (ck.keyword_id = ar.article_id) ";
        $q .= "WHERE ar.article_status  = '".$article_status."' ";
        $q .= "AND ck.campaign_id='".$campaign_id."' AND ".$role_field."='".$user_id."' ";
