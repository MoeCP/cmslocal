...
class Candidate
{
    ...
    function save($bind)
    {
        ...
        if (isset($bind['pay_pref']) &&!empty($bind['pay_pref']) &&$bind['pay_pref'] == 3 &&empty($bind['paypal_email'])) {
            $feedback = 'Please specify Paypal Email';
            return false;
        }
        ...
        if (isset($bind['cpermission']) && empty($bind['cpermission'])) {
            $feedback = 'Please specify the editorial team';
            return false;
        }


        if (isset($bind['clocation']) && empty($bind['clocation'])) {
            $feedback = 'Please specify you are a citizen of one of these countries';
            return false;
        }
      ...
      foreach ($bind as $k => $row) {
        ...
        if (!empty($result)) {
            $result = $result[0];
            if ($candidate_id > 0 && $candidate_id == $result['candidate_id'] || $bind['cpermission'] <> $result['cpermission']) {
            } else {
                $feedback = 'Duplicated email, please to check';
                return false;
            }
        }*/
        $result = self::duplicatedEmail(array('email' => $bind['email'], 'cpermission' => $bind['cpermission'], 'candidate_id' => $candidate_id));
        if (!empty($result)) {
            $feedback = 'Duplicated email, please to check';
            return false;
        }
        ...
        // send email to community@copypress.com
        if ($result > 0 && $is_new) {
            global $mailer_param, $g_payment_preference;
            $subject = 'New application Submitted';
            $body  = "First Name : {$bind['first_name']}<br />";
            $body .= "Last Name : {$bind['last_name']}<br />";
            $body .= "Email Address : {$bind['email']}<br />";
            $body .= "Birthday : {$bind['dob']}<br />";
            $body .= "Address : {$bind['address']}<br />";
            $body .= "City : {$bind['city']}<br />";
            $body .= "State : {$bind['state']}<br />";
            $body .= "Zip : {$bind['zip']}<br />";
            $body .= "Country : {$bind['country']}<br />";
            $body .= "Gender : {$bind['sex']}<br />";
            $body .= "First language : {$bind['first_language']}<br />";
            $body .= "Weekly Hours : {$bind['weekly_hours']}<br />";
            $body .= "Preferred Method of Payment : {$g_payment_preference[$bind['pay_pref']]}<br />";
            if (!empty($bind['paypal_email'])) $body .= "Paypal Email Address : {$bind['paypal_email']}<br />";
...
/*        $result = self::__getResult(array('email' => $bind['email']));
        ...
        $result = self::duplicatedEmail(array('email' => $bind['email'], 'cpermission' => $bind['cpermission']));
        if (!empty($result)) {
            $feedback = 'Duplicated email, please to check';
            return false;
        }
    ...
    function sentEmail($bind, $event_id = 16)
    {
        ...
        $body = email_replace_placeholders($info['body'], array());
        $body = nl2br($body);
        //echo $body;
    ...
    public function update($bind)
    {
        ...
        if (isset($bind['candidate_id']) && $bind['candidate_id']) {
            if (isset( $bind['email'])) {
                $result = self::duplicatedEmail(array('email' => $bind['email'], 'cpermission' => $bind['cpermission'], 'candidate_id' => $bind['candidate_id']));
                if (!empty($result)) {
                    $feedback = 'Duplicated email, please to check';
                    return false;
                }
    ...
    function duplicatedEmail($p)
    {
        ...
        if (isset($p['email'])) {
            ...
            $cpermission = addslashes(htmlspecialchars(trim($p['cpermission'])));
            if(!empty($email))
                $condition[] = "email='{$email}' and cpermission = '{$cpermission}'";
        }
