...
class ArticlePaymentLog{
    ...
    function monthlyReport($type = 'copy writer', $p = array(), $is_pagination = true)
    {
        ...
        if (isset($p['date_assigned']) && !empty($p['date_assigned'])) {
            $astimestamp = strtotime($p['date_assigned']."-08");
            $assignment_date_start = date('Y-m-01 00:00:00', $astimestamp);
            $assignment_date_end = date('Y-m-t 23:59:59', $astimestamp);
            $conditions[] = 'ck.date_assigned<= \'' . $assignment_date_end . '\'';
            $conditions[] = 'ck.date_assigned>= \'' . $assignment_date_start . '\'';
        }
    ...
    function batchUpdatePaymentMonth($p) {
        global $conn;

        //##$pay_month = addslashes(htmlspecialchars(trim($p['forcec_pay_month'])));
        $pay_month = (int)$p['forcec_pay_month'];
        $keywrodids = Campaign::getKeywordsIDs($p);
        if (!empty($keywrodids)) {
            $sql = "UPDATE article_payment_log SET pay_month = '$pay_month' ".
                   "WHERE (paid_time IS NULL OR paid_time='0000-00-00 00:00:00') ".
                   "AND article_id IN (". implode(',', $keywrodids) . ") ";
