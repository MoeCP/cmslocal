...
class Campaign {
    ...
    function add($p = array())
    {
        ...
        if (strlen($max_word) == 0) {
            ...
            return false;
        } else if (!is_numeric($max_word) || ($max_word < 50 || $max_word > 1000) && $max_word <> 0) {
             $feedback = "Please input number between 50 and 1000. if it's no limit, please input 0";
            return false;
        }
    ...
    {
        global $conn, $mailer_param, $g_to_email;
...
//        $conn->Execute($q);
    ...
    function getAssignedKeywords($p = array())
    {
        ...
        list($from, $to) = $pager->getOffsetByPageId();
        $q =" SELECT `cc`.`campaign_name`, ck.editor_status, ck.cp_status, ck.editor_id, ck.copy_writer_id, ck.keyword, ar.article_number, ar.article_status, ck.article_type,ck.keyword_id, ck.campaign_id, ar.article_id, ck.date_start, ck.date_end, ck.date_assigned, ck.cp_accept_time  ,CONCAT(ue.first_name, ' ', ue.last_name) AS ue_name, ue.user_name AS ue_name,  cp.user_name AS cp_name, CONCAT(cp.first_name, ' ', cp.last_name) AS cp_name " 
            . $from_q
        ...
        if ($rs) {
            while (!$rs->EOF) {
                ...
                if ((($role == 'admin' || $role == 'project manager') && ($fields['cp_status']== 0 || $fields['editor_status'] == 0)|| $role == 'editor'  && $fields['editor_status'] == -1   || $role == 'copy writer' && $fields['cp_status'] == -1) ) { // && $fields['cp_status'] == 1
    ...
    function searchKeyword($p = array(), $show_kd_groupby_topic = false)
    {
        ...
        */
        ...
        $q = "SELECT DISTINCT `ck`.`keyword_id`, `ck`.`campaign_id`, `cc`.`campaign_name`, `ck`.`copy_writer_id`, \n". 
            ...
            "`ck`.`date_start`, `ck`.`date_end`, `ck`.`creation_user_id`, `ck`.`creation_role`, `ck`.`cp_status`,`ck`.`cp_accept_time`, `ck`.`editor_status`,ck.subcid,  \n" .
            ...
            "CONCAT(uc.first_name, ' ', uc.last_name) AS uc_name, uc.pay_level, CONCAT(ue.first_name, ' ', ue.last_name) AS ue_name , cu.user_name AS cu_name, ar.body, ar.richtext_body  \n" . 
             "FROM campaign_keyword AS ck \n".
             "LEFT JOIN articles AS ar ON (ar.keyword_id = ck.keyword_id) ".
             $sqls['left'] .
            ...
            foreach ($campaigns as $k => $value) 
            {
                ...
                if ($campaign_id > 0) {
                    ...
                    if (!empty($value['acceptance_desc'])) {
                        $keyword_list_str .= "Assignment Acceptance: " . html_entity_decode($value['acceptance_desc']) . "<br />";
                    }
                    // end
