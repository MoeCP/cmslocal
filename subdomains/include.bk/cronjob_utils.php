...
function get_dup_links($obj)
{
    ...
    for ($j=0;$j<$limit_dup_url;$j++) {
        if (isset($obj['result'][$j])) {
            ...
            $dup_link .= "Textsnippet:&nbsp;{$rs->textsnippet}<br />";
        }
    }
    return $dup_link;
}
...
function sent_duplicated_article_email($cpinfo, $dup_links, $keyword, $article_key, $editor)
{
    ...
    // $ccs = array('contentmanager@secondstepsearch.com', 'tony@infinitenine.com');
    $ccs = array('contentmanager@secondstepsearch.com');
...
function copyscope_credit_alert($error)
{
    global $mailer_param;
    $body = $error;
    $subject = 'Copyscope Api Error Alert';
