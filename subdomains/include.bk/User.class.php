    ...
    function getAccountingConditionOrSql($p = array(), $role = 'copy writer')
    {
        ...
        case "export":
            ...
            if ($type == 'multi-cp' || $type == 'export')
            {
                if ($type == 'export') {
                    ...
                    $user_sql .= "u.pay_pref, u.form_submitted, u.notes, u.status, cph.payment_flow_status, cph.memo, ";
        ...
        case "article-amount":
            ...
            case 'invoice':
                ...
                $sql_select .= " IF (ach.article_type_name != '' && ach.article_type_name IS NOT NULL, ach.article_type_name, at.type_name) AS article_type_name, at.parent_id AS at_parent_id "; 
                $sql_from .= "\n LEFT JOIN users AS u ON ({$field} = u.user_id) ";
            ...
            case 'type-cost':
                ...
                break;
            case 'article-amount':
                $q  = "SELECT SUM(ar.total_words) AS num, COUNT(ar.article_id) AS total , ck.article_type, at.parent_id AS at_parent_id, ck.campaign_id, {$field} AS user_id, \n";
