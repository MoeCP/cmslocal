...
class CampaignKeyword{
    ...
    function _updateArticleStatusById($fields, $id, $campaign_id, $api_info)
    {
        ...
        $sql = "SELECT ck.copy_writer_id, ck.editor_id, ck.campaign_id, ar.is_canceled, ck.article_type, ar.article_id, ar.article_status, ar.title, ar.current_version_number AS version,ar.total_words AS length, ar.body as textBody, ar.richtext_body AS htmlBody FROM campaign_keyword AS ck, articles AS ar WHERE ar.keyword_id=ck.keyword_id AND ar.article_id='" . $id . "' ";
        ...
        if (is_array($result)) {
            foreach ($result as $k => $row) {
                ...
                if ($status == 5 || $status == 6) {
                    ...
                    $result[$k]['status'] = 'Pending Review';
                } else if ($status == 0 && empty($row['copy_writer_id'])) {
                    $result[$k]['status'] = 'Unassigned';
                } else if ($row['copy_writer_id'] > 0) {
                    $result[$k]['status'] = 'Writing';
                }
        ...
        if (is_array($result)) {
            foreach ($result as $k => $row) {
                ...
                if ($status == 6  || $status == 5) {
                    ...
                    $result[$k]['status'] = 'Not Ready';
                    /*unset($result[$k]['title']);
                    unset($result[$k]['textBody']);
                    unset($result[$k]['length']);
                    unset($result[$k]['htmlBody']);*/
                }
                unset($result[$k]['copy_writer_id']);
            }
        } else {
    ...
    function getAllCampaigns($api_info, $fields = array())
    {
        ...
        return $this->__getCampaignInfo($arr, $conditions);
    }

    // added by nancy xu 2010-09-30 19:25
    function __getCampaignInfo($fields, $conditions)
    {
        global $conn;
    ...
    function getArticleType($param = array(), $api_info = array()){
        ...
        $sql ='SELECT type_name AS typeName, type_id AS typeId FROM  article_type WHERE parent_id >= 0';
        $result = $conn->GetAll($sql);
