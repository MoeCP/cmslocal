	...
	$(document).ready(function(){
		...
		var widget = {
			config: {
				...
				audio_html: '<audio id="ringfx"><source src="//content.copypress.com/js/agora/assets/ring.ogg" type="audio/ogg"><source src="//content.copypress.com/js/agora/assets/ring.mp3" type="audio/mpeg"><source src="//content.copypress.com/js/agora/assets/ring.wav" type="audio/x-wav"></audio><audio id="noticefx"><source src="//content.copypress.com/js/agora/assets/notice.ogg" type="audio/ogg"><source src="//content.copypress.com/js/agora/assets/notice.mp3" type="audio/mpeg"><source src="//content.copypress.com/js/agora/assets/notice.wav" type="audio/x-wav"></audio>',
			},
			...
			*/
			createMainWidget: function(cb){
				...
				setTimeout(function(){
					...
					// Import Widget HTML
					setTimeout(function(){
						self.mainWidgetBody.html(self.config.main_widget_html + self.config.audio_html);
			...
			setMainWidgetEventHandler: function(cb){
				...
				// Main Widget List Item Click
				self.mainWidgetContents.find('ul.dropdown-menu li').on('click', function(e){
					var selfli = this;
					if (typeof($(this).attr("data-status")) == "undefined") {
						...
						$.ajax({
							...
							'data':{act:"setStatus",user_id:self.config.user_id,chat_status:$(this).attr("data-status")},
							'success':function(data){
								//$(".js-chat-status").removeClass("glyphicon-ok-sign glyphicon-minus-sign glyphicon-time glyphicon-off");
								/*
								var _new_class;
								switch($(selfli).attr("data-status")) {
									case "1":
									  _new_class = "glyphicon glyphicon-ok-sign";
									  break;
									case "2":
									  _new_class = "glyphicon glyphicon-minus-sign";
									  break;
									case "3":
									  _new_class = "glyphicon glyphicon-time";
									  break;
									case "4":
									  _new_class = "glyphicon glyphicon-off";
									  break;
									default:
									  _new_class = "glyphicon glyphicon-ok-sign";
								}
								*/
								...
								var _new_class = self.getUserChatStatus($(selfli).attr("data-status"));
								//console.log(data);
								//console.log($(selfli).attr("data-status"));
								//console.log(_new_class);
								self.mainWidgetBody.find(".js-chat-status span").removeClass("glyphicon glyphicon-ok-sign glyphicon-minus-sign glyphicon-time glyphicon-off").addClass(_new_class);

								self.mainWidgetBody.find(".js-tool-dropdown").click();
							},
			...
			*/
								...
								*/
								...
								if (user_info.user_name) {
									if (user_info.user_name != user.nickname || user_info.user_image != user.picture) {
										self.mainWidgetUpdateUser(user.guest_id, user.nickname, user.picture, user.chat_status);
									}
								}
								...
								if (user.is_online) {
									...
									if (!self.setUserOnline(user.guest_id)) {
										self.mainWidgetAddUser(user.guest_id, user.nickname, user.picture, 0, user.is_online, user.chat_status);
									}
								} else {
			...
			makeNoticeSound: function(){
				//self.mainWidgetBody.find('#noticefx').play();//make noise when we get new message.
				var _doc = document.getElementById('chatboxifr' ).contentDocument || document.frames['chatboxifr'].document;
				_doc.getElementById('noticefx').play();
			},
			makeRingSound: function(){
				//make noise when we get send a audio/vedio invitation.
				/*
				var _doc = document.getElementById('chatboxifr' ).contentDocument || document.frames['chatboxifr'].document;
				_doc.getElementById('ringfx').play();

				setTimeout(function(){_doc.getElementById('ringfx').play();}, 9000);
				*/
			},
			stopRingSound: function(){
				//make noise when we get send a audio/vedio invitation.
				var _doc = document.getElementById('chatboxifr' ).contentDocument || document.frames['chatboxifr'].document;
				_doc.getElementById('ringfx').stop();

				setTimeout(function(){_doc.getElementById('ringfx').stop();}, 9000);
			},
			...
			mainWidgetUpdateUser: function(user_id, user_name, user_image, chat_status){
				var self = this;
				var user_elem = self.mainWidgetContentBodyContent.find('.channel-user[data-id="%user_id%"]'.replace('%user_id%',user_id)).first();
				...
				if (chat_status && chat_status != "0") {
					var status_class = self.getUserChatStatus(chat_status);
					status_class = '<i class="'+status_class+'"></i>';
					user_elem.find('.user-name').prepend(status_class);
				}
			},
			...
			mainWidgetAddUser: function(user_id, user_name, user_picture, unread_count, is_online, chat_status){
				var self = this;
				...
				var status_class = '';
				if (chat_status && chat_status != "0") {
					status_class = self.getUserChatStatus(chat_status);
					status_class = '<i class="'+status_class+'"></i>';
				}
				...
				if (is_online) {
				    block = '<li class="channel-user" data-is-online="%is_online%" data-id="%user_id%" data-name="%user_name%">'+videoblock+'<div class="user-unreadcount">%unread_count%</div><div class="user-name">'+status_class+'%user_name%</div></li>';
				}
			...
			bindEventToPageUser: function(){
				...
				$(".js-cp-box-user").each(function(){
					$(this).off("click").on("click",function(){
						...
						var chat_status = $(this).attr("chat-status") ? $(this).attr("chat-status") : 0;
						//console.log($(this).attr("data-name"));
						self.mainWidgetAddUser(user_id, user_name, "", 0, true, chat_status);
			...
			initLocalStream: function(id, callback){
				...
				_aws.localStream.init(function() {
					...
					if(!_aws.videoChecked){
						self.mainWidgetBody.find(".agora-screen .left div").addClass("waitAudio");
						self.mainWidgetBody.find("#agora_local div").hide();
						self.mainWidgetBody.find(".agora-screen .left #agora_local").html("<img src='"+self.config.assets_url+"/assets/audio-only.jpg' style='width:100%;height:100%;' />");
						_aws.localStream.play('agora_local', self.config.assets_url);
					} else {
						if(_aws.viewSwitch){
							$("body").append('<div id="agora_remote'+_aws.localStream.getId()+ '"></div>');
							_aws.localStream.play('agora_remote'+ _aws.localStream.getId(), self.config.assets_url);
			...
			subscribeAgoraSdkEvents: function() {
				...
				_aws.agoraClient.on('peer-leave', function(evt){
					...
					//}
					self.updateSignallingStatus('leave');
				});
				...
				_aws.agoraClient.on('stream-subscribed', function (evt) {
					...
					*/
					...
					self.updateSignallingStatus('accept');
					...
					if(!_aws.videoChecked){
						...
						stream.play('agora_remote'+stream.getId(), self.config.assets_url);
			...
			updateAgoraCalleeNotice: function() {
				...
				$.ajax({
					...
					'data': 'uid='+self.config.user_id+"&act=get",
					'success':function(data){
						var current_server_ts = 0;
						if (data['current_server_time']) current_server_ts = data['current_server_time'];
						...
						for (var i in data['channels']) {
							...
							//if the call less than 6 minus, we will make it blink notice
							if (last_call_ts && current_server_ts && (current_server_ts - last_call_ts) < 360) {
								self.agoraCalleeNotice(user.agora_caller, user.type, last_call_ts);
							}
							//if it is new/fresh call(the call was sent in 15 sec by caller. if over 15 second, it isn't fresh call) 
							if (last_call_ts && current_server_ts && (current_server_ts - last_call_ts) < 15) {
								self.makeRingSound();
							}
						}
					},
			...
			updateSignallingStatus: function(type) {
				var self = this;
				$.ajax({
					'type': 'GET',
					'dataType': 'json',
					'url': self.config.calllogs_url,
					'data': 'uid='+self.config.user_id+"&act=update&accept_type="+type,
					'success':function(data){},
					'error':function(XHR, TS, errMsg){
						console.log(TS);
						console.log(errMsg);
			...
			getUserChatStatus: function(ds) {
				...
				switch(ds) {
					case "1":
					  ...
					  break;
					case "2":
					  status_class = "glyphicon glyphicon-minus-sign";
					  break;
					case "3":
					  status_class = "glyphicon glyphicon-time";
					  break;
					case "4":
					  status_class = "glyphicon glyphicon-off";
					  break;
					default:
					  status_class = "glyphicon glyphicon-ok-sign";
				}
