...
*/
    define('BASE_DIR', dirname(dirname(QA_BASE_DIR)).'/');
    define('CMS_INC_ROOT', BASE_DIR . 'include');
...
*/
	...
	//	Until you edit this function, don't show login, register or logout links
		...
		return array(
			...
			'register' => $relative_url_prefix . '../user/user_add.php',
...
*/
	...
	//	Until you edit this function, nobody is ever logged in
		...
		if (user_is_loggedin()) {
			$user_id= User::getID();
			$level = (User::getName()=='admin') ? QA_USER_LEVEL_SUPER : QA_USER_LEVEL_BASIC;
            switch (User::getPermission()) {
            case 1:$level=QA_USER_LEVEL_EXPERT;break;
            case 3:$level=QA_USER_LEVEL_EDITOR;break;
            case 2:
            case 4:$level=QA_USER_LEVEL_MODERATOR;break;
            case 5:$level=QA_USER_LEVEL_ADMIN;break;
            }
			return array(
                'userid' => $user_id,
                'publicusername' => User::getName(),
                'email' => User::getEmail(),
                'level' => $level
            );
...
*/
	...
	*/
		...
		if (count($publicusernames)) {
			...
			$results=mysql_query(
				'SELECT user_name, user_id FROM users WHERE user_name IN ('.implode(',', $escapedusernames).')',
				$qa_db_connection
			);
	
			while ($result=mysql_fetch_assoc($results))
				$publictouserid[$result['user_name']]=$result['user_id'];
		}
...
*/
	...
	*/
		...
		if (count($userids)) {
			...
			$results=mysql_query(
				'SELECT user_name, user_id FROM users WHERE user_id IN ('.implode(',', $escapeduserids).')',
				$qa_db_connection
			);
...
*/
	...
	//	By default, show the public username linked to the Q2A profile page for the user
		...
		return '<A HREF="'.htmlspecialchars($relative_url_prefix.'../user/user_detail.php?user_id='.User::getID()).
	...
	//	By default, show the public username linked to the Q2A profile page for each user
		...
		foreach ($userids as $userid) {
			...
			if ($should_include_link)
				$usershtml[$userid]='<A HREF="'.htmlspecialchars($relative_url_prefix.'../user/user_detail.php?user_id='.$userid).
