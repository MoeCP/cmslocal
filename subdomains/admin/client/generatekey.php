...
if (!empty($_POST)) {
    ...
    if ($oClientUser->generateKey($_POST)) {
        if (!empty($pfrom)) {
            echo "<script>alert('{$feedback}');window.location.href='/client/" . $pfrom. ".php?client_id=" . $_POST['client_id'] . "'</script>";
        } else {
            if (!empty($_POST['client_user_id'])) {
                echo "<script>alert('{$feedback}');window.location.href='/client/keylist.php?client_id=" . $_POST['client_id'] . "'</script>";
            } else {
                echo "<script>alert('{$feedback}');window.location.href='/client/list.php'</script>";
            }
        }
        exit();
    }
