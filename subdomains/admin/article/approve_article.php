...
if (trim($_POST['article_id']) != ''  && trim( $_POST['approve_action'] ) != '') {
    if (!empty($_POST['invoice_date'])) $_POST['invoice_date'] = date("Y-m-d", strtotime($_POST['invoice_date']));
...
$smarty->assign('noflow_statuses', $g_tag['noflow_status']);
...
if (isset($login_role) && $login_role == 'admin' && $keyword_info["article_status"] == 4) {
    ...
    $smarty->assign('onemonthlater', changeTimeToPayMonthFormat(time()));//remove the net 30 days for now
    $smarty->assign('monthes', $monthes);
}

$cppmlog = ArticlePaymentLog::getPaymentLogByParam(array("article_id"=>$article_id,'role'=>'copy writer'));
$cppayperiod = " -- ";
if ($cppmlog && strlen($cppmlog[0]['pay_month'])==7) {
    $cppayperiod = $cppmlog[0]['pay_month'];
    $cppayperiod = substr($cppayperiod, 0, 4) . "-" . substr($cppayperiod,4,2) . "(" . substr($cppayperiod,6,1) . ")";
}
$smarty->assign('cppayperiod', $cppayperiod);
// ------------------------ End Added By Leo 11/12/2014 -------------------- //
