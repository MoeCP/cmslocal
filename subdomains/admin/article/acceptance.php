...
if (!empty($_POST)) {
    ...
    if ($permission >= 4) {
        if (!empty($_POST['isUpdate'])) {
            ...
            if (isset($_POST['qaer_id'])) $p['qaer_id'] = $_POST['qaer_id'];
...
if (!empty($_GET) || User::getPermission() == 1 || User::getPermission() == 3) {
    $search = Campaign::getAssignedKeywords($_GET);
    if ($search) {
...
if (User::getPermission() >= 4) { 
    ...
    // added by nancy xu 2014-05-06 0:13
    ...
    // end
    $all_copy_writer = User::getAllUsers($mode = 'id_name_only', $user_type = 'copy writer', true);
...
$smarty->display('article/acceptance.html');
?>