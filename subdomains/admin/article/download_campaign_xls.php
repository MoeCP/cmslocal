...
$field_labels['richtext_body']['label'] = 'Description';
...
$field_labels['category_id']['label'] = 'category';
$field_labels['article_number']['label'] = 'Article Number';
$field_labels['keyword']['label'] = 'Keyword';
//$fields = array('optional1', 'optional2', 'optional3', 'optional4', 'optional5', 'optional6', 'optional7','optional8','optional9','optional10', 'title','custom_field1', 'custom_field2', 'custom_field3', 'body');
//$fields = array('optional1', 'optional2', 'optional3', 'optional4', 'optional5', 'optional6', 'optional7','optional8','optional9','optional10', 'title','custom_field1', 'custom_field2', 'custom_field3', 'richtext_body', 'content_link');
if ($campaign_info['template'] == '2') {
    $fields = array('optional1', 'optional2', 'optional3', 'optional4', 'optional5', 'optional6', 'optional7','optional8','optional9','optional10', 'title','html_title', 'custom_field1', 'custom_field2', 'custom_field3', 'custom_field4', 'custom_field5', 'richtext_body', 'content_link','small_image', 'large_image', 'image_credit', 'image_caption', 'meta_description','blurb','category_id');
    $columns = array(
        'A', 'B', 'C', 'D', 'E', 'F', 'G', 'H', 'I', 'J', 'K','L','M','N','O','P','Q','R','S','U','V','W','X','Y','Z','AA'
    );
} else {
    $fields = array('optional1', 'optional2', 'optional3', 'optional4', 'optional5', 'optional6', 'optional7','optional8','optional9','optional10', 'title','html_title', 'custom_field1', 'custom_field2', 'custom_field3', 'custom_field4', 'custom_field5', 'richtext_body', 'article_number','blurb','content_link','keyword');
    $columns = array(
...
foreach ($all_article as $k => $item) {
    foreach ($fields as  $key => $field) {
        if ($field == 'content_link') {
            $value = base_url() . '/cms_content_source.php?sid=' .base64_encode($item['campaign_id'] . '-' . $item['keyword_id'] . '-' . $item['article_id']);
        } else {
            $value = $item[$field];
            if ($field == 'title') {
                $value = html_entity_decode($value, ENT_QUOTES, 'UTF-8');
            } else if ($field == 'richtext_body' || $field == 'body') {
				if($client_id != "350"){
					$value = stripslashes(change_richtxt_to_paintxt(html_entity_decode($value)));
				}
				else{
        ...
        if ($field == 'category_id') {
            $value = $g_2image_categories[$value];
        }
