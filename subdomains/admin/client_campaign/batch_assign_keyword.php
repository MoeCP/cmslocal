...
if (!empty($_POST['keyword_id']) && !empty($_POST['isUpdate'])) {//其实可以只用其中一个来判断
    ...
    if (Campaign::batchAssignKeyword($p)) {
        ...
        echo "<script>alert('".$feedback."');window.location.href='/client_campaign/batch_assign_keyword.php?$query_string';</script>";
        exit();
    }
}

if (!empty($_GET)) {
    $search = Campaign::searchKeyword($_GET, true);
...
if ($condition_res) {
    ...
    $smarty->assign('c_date_end', $c_date_end);
    $smarty->assign('c_date_start', $c_date_start);    
}
