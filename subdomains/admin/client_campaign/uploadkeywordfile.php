...
if (!empty($_POST)) {
    ...
    if (!empty($_FILES) && $_FILES["filename"]["error"] == UPLOAD_ERR_OK) {
        ...
        if ($ext == 'csv') {
            ...
            if ($file_id = KeywordFile::save($arr)) {
                echo "<script>window.location.href='/client_campaign/kfieldmapping.php?file_id={$file_id}';</script>";
                exit();
            }
        } else {
...
$smarty->display('client_campaign/uploadkeywordfile.html');
?>