...
// Settings
	...
	$extension_whitelist = array("jpg", "gif", "png", "xml");	// Allowed file extensions
...
// Process the file
	...
	*/
	...
	if (!@move_uploaded_file($_FILES[$upload_name]["tmp_name"], $save_path.$file_name)) {
        ...
        //echo $newxml;
        $orgxml = simplexml_load_file($save_path.$file_name, 'SimpleXMLElement', LIBXML_NOCDATA);
        ...
        if ($orgxml) {
            ...
            $newxml .= '<BulkGeoSeoLoad>';

            foreach($orgxml as $orgr)
            {
                $mapping_id = "";
                $richBody = $orgr->richBody;

                //first standardize the ticks
                /*
                $richBody = str_replace("&lsquo;&rsquo;&rsquo;", "&lsquo;&lsquo;&lsquo;", $richBody);
                $richBody = str_replace("&ldquo;&rsquo;","&lsquo;&lsquo;&lsquo;", $richBody);
                $richBody = str_replace("&ldquo;&lsquo;","&lsquo;&lsquo;&lsquo;", $richBody);
                $richBody = str_replace("&rdquo;&rsquo;","&lsquo;&lsquo;&lsquo;", $richBody);
                $richBody = str_replace("&rdquo;&lsquo;","&lsquo;&lsquo;&lsquo;", $richBody);
                $richBody = str_replace("&rsquo;&lsquo;&lsquo;", "&lsquo;&lsquo;&lsquo;", $richBody);

                //now fix ticks
                $richBody = unhtmlentities($richBody);
                //$richBody = iconv('gbk', 'utf-8', $richBody);
                $richBody = str_replace("¡¯¡¯¡¯", "'''", $richBody);
                $richBody = str_replace("¡®¡®¡®", "'''", $richBody);
                $richBody = str_replace("¡®", "'", $richBody);
                $richBody = str_replace("¡¯", "'", $richBody);
                $richBody = str_replace("¡°", '"', $richBody);
                $richBody = str_replace("¡±", '"', $richBody);
                */
                $richBody = str_replace("&lsquo;&rsquo;&rsquo;", "'''", $richBody);
                $richBody = str_replace("&ldquo;&rsquo;","'''", $richBody);
                $richBody = str_replace("&ldquo;&lsquo;","'''", $richBody);
                $richBody = str_replace("&rdquo;&rsquo;","'''", $richBody);
                $richBody = str_replace("&rdquo;&lsquo;","'''", $richBody);
                $richBody = str_replace("&rsquo;&lsquo;&lsquo;", "'''", $richBody);

                $richBody = str_replace("&lsquo;", "'", $richBody);
                $richBody = str_replace("&rsquo;", "'", $richBody);
                $richBody = str_replace("&ldquo;", '"', $richBody);
                $richBody = str_replace("&rdquo;", '"', $richBody);
                $richBody = str_replace("&nbsp;", " ", $richBody);

                //fix other stuff
                //$richBody = eregi_replace('<br[[:space:]]*/?[[:space:]]*>', chr(13).chr(10), $richBody);//br2nl
                $richBody = preg_replace('/\<br(\s*)?\/?\>/i', "\r\n", $richBody);//br2nl

                $richBody = str_replace("<p>", "", $richBody);
                $richBody = str_replace("</p>", "\r\n\r\n", $richBody);

                $richBody = preg_replace("/<a .*?href=([\"|\']*)(.*)?\\1.*?>(.*)?<\/a>/i", "[[\\2 \\3]]", $richBody);;

                if (!empty($orgr->mappingID))
                    $mapping_id = " indexHierarchy=\"{$orgr->mappingID}\"";

                $newxml .= "<GeoSearchEngineOptimization{$mapping_id}>";
                $newxml .= "<DestinationIntro><![CDATA[";
                //$newxml .= $orgr->richBody;
                $newxml .= $richBody;
                $newxml .= "]]></DestinationIntro>";
                $newxml .= "</GeoSearchEngineOptimization>";
            }
